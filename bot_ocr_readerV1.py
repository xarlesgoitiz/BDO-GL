...
def share_sheet_with_email(email: str):
    ...
    try:
        ...
        return f'La hoja de cálculo ha sido compartida con emailaso@emailuko.ema :spy: con permisos de edición.'
...
def procesar_imagen(image_path):
    ...
    try:
        ...
        if len(result) > 0:
            ...
            enemy_result, date, time, enemy_guild = parse_header(extracted_text[0])
            guild_result, guild = re.match(r'\[(Victory|Defeat)\](.+)', extracted_text[1]).groups()
            enemy_result, enemy_guild = re.match(r'\[(Victory|Defeat)\](.+)', extracted_text[2]).groups()
            ...
            for i in range(3, len(extracted_text), 6):
                players.append(extracted_text[i])
                k, d = separar_kills_deaths(extracted_text[i + 1])
                kills.append(k)
                deaths.append(d)
                debuffs.append(extracted_text[i+2])
                dealt.append(extracted_text[i+3])
                taken.append(extracted_text[i+4])
                healed.append(extracted_text[i+5])
